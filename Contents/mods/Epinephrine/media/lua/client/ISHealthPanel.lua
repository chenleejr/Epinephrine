...
function ISHealthBodyPartListBox:drawTextAccordingToKCEpinephrine(originalText, additionalText, x, y)
    if KCEpinephrine.data.takingEffect then
        self:drawText("- " .. getText(originalText) .. additionalText .. getText("IGUI_ignored_by_KCEpinephrine"), x, y, 0.28, 0.28, 0.89, 1, UIFont.Small);
    else
        self:drawText("- " .. getText(originalText) .. additionalText, x, y, 0.89, 0.28, 0.28, 1, UIFont.Small);
    end
end
...
function ISHealthBodyPartListBox:doDrawItem(y, item, alt)
    ...
    if bodyPart:scratched() then
        ...
        self:drawTextAccordingToKCEpinephrine("IGUI_health_Scratched", text, x, y)
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            if(bodyPart:getScratchTime() > 0) then
    ...
    if bodyPart:isCut() then
        ...
        self:drawTextAccordingToKCEpinephrine("IGUI_health_Cut", text, x, y)
        y = y + fontHgt;
        if ISHealthPanel.cheat then
    ...
    if bodyPart:deepWounded() then
        ...
        self:drawTextAccordingToKCEpinephrine("IGUI_health_DeepWound", text, x, y)
        y = y + fontHgt;
        if ISHealthPanel.cheat then
    ...
    if bodyPart:bitten() then
        self:drawTextAccordingToKCEpinephrine("IGUI_health_Bitten", "", x, y)
        y = y + fontHgt;
        if ISHealthPanel.cheat then
    ...
    if bodyPart:getAdditionalPain() > 10 then
        if bodyPart:getAdditionalPain() > 50 then
            self:drawTextAccordingToKCEpinephrine("IGUI_health_HeavyPain", "", x, y)
        else
            self:drawTextAccordingToKCEpinephrine("IGUI_health_Pain", "", x, y)
        end
        y = y + fontHgt;
        if ISHealthPanel.cheat then
    ...
    if bodyPart:getBurnTime() > 0 and not bodyPart:bandaged() then
        ...
        self:drawTextAccordingToKCEpinephrine("IGUI_health_Burned", burnText, x, y)
        y = y + fontHgt;
        if ISHealthPanel.cheat then
